...
exports.getsearchbyid = (req, res, next) => {
    ...
    // res.render('admin/searchid', { pagetitle: 'searchid' })
    res.render('education/searchbyid', { pagetitle: 'edit detail', errormessage: message });
}
...
exports.postsearchbyid = (req, res, next) => {
    ...
    Admin.findOne({ email: email }).then(data => {
        if (data) {
            console.log(data);
            res.render('education/updateprofile', { pagetitle: 'All detail', studentdata: data })
        }
        else {
            req.flash('error', 'Id Dose not Found');
            res.redirect('/edu/searchbyid');
        }
    })
...
exports.posteditdetail = (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
        return res.status(402).render('education/addinstitute', { pagetitle: 'Add Profile', path: '/edu/addinstitute', editing: true, errormessage: errors.array()[0].msg, product: { name: name, mobileno: mobileno, indexno: indexno, email: email, password: password } })
    } else {
...
exports.getsearchbox = (req, res, next) => {
    ...
    // res.render('admin/searchid', { pagetitle: 'searchid' })
    res.render('education/studentsearchid', { pagetitle: 'edit detail', errormessage: message });
}
...
exports.getstudentinformation = (req, res, next) => {
    ...
    Student.findOne({ adharno: adharno }).then(data => {
        res.render('education/studentdetail', { pagetitle: 'student detail', editing: true, product: data, errormessage: message })
    }).catch(err => {
        console.log(err);
    })
}
exports.postupdatedetail = (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
        console.log(xender);
        return res.status(402).render('education/studentdetail', { pagetitle: 'Add Profile', path: '/edu/studentdetail', editing: true, errormessage: errors.array()[0].msg, product: { name: name, mobileno: mobileno, Dob: dob, xender: xender, adharno: adrno, email: email, password: password, adminid: adminid } })
    } else {
